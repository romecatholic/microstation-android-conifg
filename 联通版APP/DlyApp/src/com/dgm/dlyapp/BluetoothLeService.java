...
public class BluetoothLeService extends Service {
    ...
    public volatile String Service_uuid = "69400001-b5a3-f393-e0a9-e50e24dcca99";//charaterID
    //public static String Characteristic_uuid_TX = "0000ffe1-0000-1000-8000-00805f9b34fb";
    public volatile String Characteristic_uuid_TX = "69400002-b5a3-f393-e0a9-e50e24dcca99";//write
    public volatile String Characteristic_uuid_FUNCTION = "69400003-b5a3-f393-e0a9-e50e24dcca99";//notify
	
    private Handler mHandler;
    private final static String UUID_KEY_DATA = "69400001-B5A3-F393-E0A9-E50E24DCCA99";
	...
	public void sendCmd(byte [] b){
		try{
			...
			BluetoothGattCharacteristic gg;
			if(mBluetoothGatt.getService(UUID.fromString(Service_uuid))!=null){
				gg=mBluetoothGatt.getService(UUID.fromString(Service_uuid)).getCharacteristic(UUID.fromString(Characteristic_uuid_TX));
				//String s="7e323030313430343730303030464441450d";
				gg.setValue(b);
				mBluetoothGatt.writeCharacteristic(gg);
				isSuccess=true;
			}
...
//	}
    ...
    public void enable_JDY_ble( boolean p){
		...
		if(p)
	    {
			...
			BluetoothGattCharacteristic ale=null;// =service.getCharacteristic(UUID.fromString(Characteristic_uuid_TX));
			switch( 0 )
			{
				case 0://0xFFE1 //͸��
				{
			...
			if(ale!=null){
				...
				Log.d(TAG," setnotification = " + set);
				BluetoothGattDescriptor dsc =ale.getDescriptor(UUID.fromString(  "00002902-0000-1000-8000-00805f9b34fb"));
				/*BluetoothGattDescriptor dsc =ale.getDescriptor(UUID.fromString(Characteristic_uuid_FUNCTION));
				if(dsc==null){
					dsc=new BluetoothGattDescriptor(UUID.fromString(Characteristic_uuid_FUNCTION), BluetoothGattDescriptor.PERMISSION_READ);
				}
				...
				*/
				...
				/*List<BluetoothGattDescriptor> descriptors=ale.getDescriptors();
				for(int i=0;i<descriptors.size();i++){
					Log.e("aaa", descriptors.get(i).getUuid().toString());
				}
...
//        	mBluetoothGatt.setCharacteristicNotification(jdy, p);
    ...
    public void displayGattServices(List<BluetoothGattService> gattServices) {
    	...
    	for (BluetoothGattService gattService : gattServices) {
    	    ...
    	    if(gattService.getUuid().toString().equals(DataAssembleUtil.DGMUUID)){//如果是大光明的uuid
    	    	this.Service_uuid = DataAssembleUtil.DGMUUID;
    	        this.Characteristic_uuid_TX = DataAssembleUtil.DGMWRITE;
    	        this.Characteristic_uuid_FUNCTION = DataAssembleUtil.DGMNOTIFY;
    	        DataAssembleUtil.isJAUUID=1;
    	        return;
    	    }else if(gattService.getUuid().toString().equals(DataAssembleUtil.JAUUID)){
    	    	this.Service_uuid = DataAssembleUtil.JAUUID;
    	    	this.Characteristic_uuid_TX = DataAssembleUtil.JAWRITE;
    	    	this.Characteristic_uuid_FUNCTION = DataAssembleUtil.JANOTIFY;
    	    	DataAssembleUtil.isJAUUID=0;
    	    	return;
    	    }
    	    ...
    	    for (final BluetoothGattCharacteristic  gattCharacteristic: gattCharacteristics) {
    	        ...
    	        Log.e(TAG,"---->char permission:"+getCharPermission(permission));
    	        Log.e(TAG,"---->char permission:"+permission);
    	
    	        int property = gattCharacteristic.getProperties();
    	        Log.e(TAG,"---->char property:"+getCharPropertie(property));
    ...
    public String getCharPropertie(int property){
        return getHashMapValue(charProperties,property);
    }
    
    private String getCharPermission(int permission){
    ...
    private String getHashMapValue(HashMap<Integer, String> hashMap,int number){
	...
	private List<Integer> getElement(int number){
