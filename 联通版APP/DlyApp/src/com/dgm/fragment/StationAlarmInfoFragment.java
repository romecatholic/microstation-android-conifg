...
public class StationAlarmInfoFragment extends Fragment {
...
//		}
		...
		new LoadDataAsyncTask(StationAlarmInfoFragment.this).execute(url + pageNo);
...
//							PullToRefreshBase<ListView> refreshView) {
//						Log.e("@@@@@@@@", "" + pageNo);
//						new LoadDataAsyncTask(StationAlarmInfoFragment.this).execute();
//
//						pd.setIndeterminate(true);
...
//						Fragment newContent = new AlarmStationFragment();
////						Bundle args = new Bundle();
////						args.putString("stationId", stationid);
////						args.putString("backButton", "");
////						newContent.setArguments(args);
//						String title = "alarminfo";
//						switchFragment(newContent, title);
//					}
//
//				});
	...
	private class LoadDataAsyncTask extends AsyncTask<String, Void, String> {
		...
		private StationAlarmInfoFragment stationAlarmInfoFragment;

		public LoadDataAsyncTask(StationAlarmInfoFragment stationAlarmInfoFragment) {
			this.stationAlarmInfoFragment = stationAlarmInfoFragment;
		}
		...
		protected String doInBackground(String... arg0) {
			String IP = "http://";
			String IPPORT = dbMgr.queryInterface();
			String url = IP+ IPPORT+"/DlyAppSever/StationAlarmServlet";
...
/*			try {
			...
			list.add(new BasicNameValuePair("stationId",StationId));//搜索告警基站名称，模糊查询
...
//			list.add(new BasicNameValuePair("operateState", "0"));//未确认告警状态位
...
//			list.add(new BasicNameValuePair("userArea", userArea));// 
		...
		protected void onPostExecute(String result) {
			...
			if (result != null) {
				...
				stationAlarmInfoFragment.pd.dismiss();
				stationAlarmInfoFragment.refresh_lv.onRefreshComplete();
			} else {
				stationAlarmInfoFragment.refresh_lv.onRefreshComplete();
			}
		}
