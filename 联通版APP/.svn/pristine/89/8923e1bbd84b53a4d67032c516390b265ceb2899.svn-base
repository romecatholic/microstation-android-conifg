...
public class FileSelectorActivity extends Activity{
	public final static int FileNameResult=1;
...
private OnTouchListener onTouchListener=new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event)
		{
			...
			default:
				v.setBackgroundColor(Color.parseColor("#ffF0F3F6"));
				break;
			}event.getAction();
			return false;
		}
	...
	private void initUI(){
		...
		llFileholder=(LinearLayout)findViewById(R.id.fileselectorListView);
	...
	private void createViewDynamically() {
		...
		for(int i=0;i<arFiles.length;i++){
			View v=inflater.inflate(R.layout.singlefolder, null);
			v.setTag(i);
			...
			if(arFiles[i].isFile()){
				...
				v.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						//Toast.makeText(getApplicationContext(), selectedFile.getAbsolutePath()+"\\"+selectedFile.getName(), Toast.LENGTH_LONG).show();
						
						Intent intent=new Intent();  
						intent.putExtra("filename", selectedFile.getAbsolutePath()+"\\"+selectedFile.getName());  
					    setResult(FileNameResult, intent);  
					    finish(); 
					}
				});
	...
	protected void highliteSelectedFile(View v,int pos){
		...
		v.setBackgroundColor(Color.parseColor("#ff83bdd5"));
		if(lastSelected!=-1){
			llFileholder.getChildAt(lastSelected).setBackgroundColor(Color.parseColor("#ffF0F3F6"));
		}
		lastSelected=pos;
	}
